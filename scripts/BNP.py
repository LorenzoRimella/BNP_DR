...
# Step 1
def step_1(one_n_j, K_current, one_hot_X, pi_i_0k_full, theta_k_full, beta_k_00_full, alpha_0, alpha_i, seed_s1):

    seed_s1_1, seed_s1_2 = tfp.random.split_seed( seed_s1, n=2, salt='step_1_split')

    Z_ij = step_1_old_mixtures(one_n_j, one_hot_X, pi_i_0k_full, theta_k_full, seed_s1_1)
...
def BNP_MCMC_initialization(a_0, b_0, a, b, one_n_j, n_indiv, K_max, K_initial, seed_initialization_single_step):
    ...
    # theta
    seed_sim, seed_carry = tfp.random.split_seed( seed_sample_initialize[1], n=2, salt='seed_initialization_theta')
    theta_k_full  = tfp.distributions.Dirichlet(concentration = one_n_j).sample(K_max, seed = seed_sim)
    while tf.reduce_any(tf.math.is_nan(theta_k_full)):
        seed_sim, seed_carry = tfp.random.split_seed( seed_carry, n=2, salt='seed_initialization_theta')
        theta_k_full  = tfp.distributions.Dirichlet(concentration = one_n_j).sample(K_max, seed = seed_sim)

    # alpha_i
    alpha_i       = tfp.distributions.Gamma( concentration = a, rate = b).sample(n_indiv, seed = seed_sample_initialize[2])

    # beta
    seed_sim, seed_carry = tfp.random.split_seed( seed_sample_initialize[3], n=2, salt='seed_initialization_beta')
    beta_k_00_full= tfp.distributions.Dirichlet(concentration = tf.concat((tf.ones((K_initial+1)), 
                                                                           tf.zeros((K_max - K_initial))), axis = 0)).sample(seed = seed_sim)
    while tf.reduce_any(tf.math.is_nan(beta_k_00_full)):
        seed_sim, seed_carry = tfp.random.split_seed( seed_carry, n=2, salt='seed_initialization_beta')
        beta_k_00_full= tfp.distributions.Dirichlet(concentration = tf.concat((tf.ones((K_initial+1)), 
                                                                            tf.zeros((K_max - K_initial))), axis = 0)).sample(seed = seed_sim)

    # pi
    seed_sim, seed_carry = tfp.random.split_seed( seed_sample_initialize[4], n=2, salt='seed_initialization_beta')
    pi_i_0k_full  = tfp.distributions.Dirichlet(concentration = tf.concat((tf.ones((K_initial+1)), 
                                                                            tf.zeros((K_max - K_initial))), axis = 0)).sample(n_indiv, seed = seed_sim)
    while tf.reduce_any(tf.math.is_nan(pi_i_0k_full)):
        seed_sim, seed_carry = tfp.random.split_seed( seed_carry, n=2, salt='seed_initialization_beta')
        pi_i_0k_full  = tfp.distributions.Dirichlet(concentration = tf.concat((tf.ones((K_initial+1)), 
                                                                            tf.zeros((K_max - K_initial))), axis = 0)).sample(n_indiv, seed = seed_sim)
...
def BNP_MCMC_step(one_hot_X, one_n_j, K_max, prior_parameters, initialization_MCMC, seed_step, type_1, type_2):
    ...
    beta_k_00_full, pi_i_0k_full, Z_ij, K_current = step_1(one_n_j, K_current, one_hot_X, pi_i_0k_full, theta_k_full, beta_k_00_full, alpha_0, alpha_i, seed_1)
...
def BNP_population_sampler(MCMC_output, one_n_j, J, batch_size, seed_pop_sampler):
    ...
    theta_0k_full = tf.concat((tf.expand_dims(one_n_j/tf.reduce_sum(one_n_j, axis =1, keepdims=True), axis = 0), theta_k_full), axis = 0)
...
def BNP_population_sampler_fixed_alpha(MCMC_output, one_n_j, J, batch_size, seed_pop_sampler):
    ...
    new_alpha_i = alpha_i[0]*tf.ones(batch_size)
...
def step_tau_fixed_alpha(MCMC_output, X_ij, one_n_j, J, batch_size, N, seed_step_tau):
    ...
    def body(iteration, input):
        ...
        X_ij_unobserved = BNP_population_sampler_fixed_alpha(MCMC_output, one_n_j, J, batch_size, seed_step_tau_splitted[iteration])
    ...
    X_ij_unobserved_final = BNP_final_population_sampler_fixed_alpha(MCMC_output, one_n_j, J, batch_size, final_size, seed_step_tau_splitted[-1])
    freq_final = fast_frequency_batched_final_size(subX_ij, X_ij_unobserved_final, one_n_j)
...
def prob_estimator_fixed_alpha(X_ij, one_n_j, alpha, theta_k_full, beta_k_00_full, mc_sample_size, seed_step_tau):
    ...
    sub_alpha_i = alpha*tf.ones(mc_sample_size)
...
def step_mc_tau_fixed_alpha(MCMC_output, X_ij, one_n_j, N, mc_sample_size, seed_step_tau):
    ...
    alpha = alpha_i[0]
    mc_sample_prob_list = [prob_estimator_fixed_alpha(X_ij, one_n_j, alpha, theta_k_full, beta_k_00_full, mc_sample_size, seed_step_tau_to_split[i]) for i in range(5)]
    mc_sample_prob   = tf.concat(mc_sample_prob_list, axis = 0)
    ...
    def body(input, t):
        ...
        if type_1=="single":
            if type_tau=="sampling":
                tau = step_tau_fixed_alpha(output_t, X_ij, one_n_j, J, batch_size, N, seed_step_with_tau)
                
            if type_tau=="Monte Carlo":
                tau = step_mc_tau_fixed_alpha(output_t, X_ij, one_n_j, N, batch_size, seed_step_with_tau)
...
#     new_K_current = tf.cast(tf.where(tf.reduce_sum(pi_i_0k_full, axis = 0)!=0)[-1], dtype = tf.int32)[0]
