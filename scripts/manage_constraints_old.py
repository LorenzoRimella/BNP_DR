...
# def expanded_mu(S_d, List_to_check, mu, components_to_expand_current, domain_to_expand_current, CompareList_components_current):
# 	new_mu_list = []
# 	new_mu = mu
...
# 					ComparList = tf.gather(S_d, nonempy_intersection_index(S_d, new_mu), axis = 0)
# 					if tf.shape(ComparList)[0]==0:
# 						new_mu_list.append(new_mu)
                     
# 				else:
# 					array_to_check = tf.concat(List_to_check, axis = 0)
# 					ComparList = tf.gather(array_to_check, nonempy_intersection_index(array_to_check, new_mu), axis = 0)
...
def expanded_mu(S_d, List_to_check, mu, components_to_expand_current, domain_to_expand_current, CompareList_components_current):
	...
	for i in range(tf.shape(domain_to_expand_current)[0]):
		...
		for j in range(i, tf.shape(domain_to_expand_current)[0]):


		for new_state in range
		new_mu = new_mu + increment_current_current
...
def create_mu_to_add(S_d, mu, components_to_expand_list, domain_to_expand_list, CompareList_components_list):
	expanded_mu_list = []
	...
	for i in range(len(components_to_expand_list)):
		...
		mu_to_add = expanded_mu(S_d, expanded_mu_list, mu, components_to_expand_current, domain_to_expand_current, CompareList_components_current)

		if tf.shape(mu_to_add)[0]!=0:
			expanded_mu_list.append(mu_to_add)
...
def iteration_disjoint_constraint(domain_j, current_constraint):
    ...
    while tf.shape(Pending)[0]!=0:
        ...
        if tf.shape(ComparList)[0]==0:
            ...
            mu_to_add = create_mu_to_add(S_d, mu, components_to_expand_list, domain_to_expand_list, CompareList_components_list)
